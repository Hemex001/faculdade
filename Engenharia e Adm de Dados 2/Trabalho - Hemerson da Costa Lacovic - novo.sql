...
DELIMITER $$
create or replace
procedure reajuste(in descricao varchar(60), in perc decimal(11,2), in dt_inicio date, in dt_fim date, in classificacao varchar(3))
begin
...
/* Insere dados na tabela `correcao_valor_produto` */
call reajuste ('Reajuste fevereiro ~ março 2024', 1.4, '2024-02-01', '2024-03-31', '005');
...
begin
	declare valor_reajustado decimal(11,2);

	select 
		case
			when cvp.cod_correcao is null then p.VALOR * p.QUANTIDADE
			else ((p.VALOR * cvp.percentual) * p.QUANTIDADE)
...
select
	...
	valor_atualizado(p.CODIGO, v.codvenda) as 'Total Reajustado'
	from vendas v
...
end as 'Total Reajustado'
from produto p
left join correcao_valor_produto cvp on cvp.CODIGO_CLASSIFICACAO = p.CODIGO_CLASSIFICACAO  
and now() between cvp.dt_inicio and cvp.dt_fim;
...
select
	v.codvenda, 
	v.datavenda, 
	sum(p.valor * p.QUANTIDADE) as 'Total',
	sum(valor_atualizado(p.CODIGO, v.codvenda)) as 'Total Atualizado'
	from vendas v
inner join itens_venda iv on v.codvenda = iv.codvenda 
inner join produto p on iv.codproduto = p.CODIGO
left join correcao_valor_produto cvp on cvp.CODIGO_CLASSIFICACAO = p.CODIGO_CLASSIFICACAO  
